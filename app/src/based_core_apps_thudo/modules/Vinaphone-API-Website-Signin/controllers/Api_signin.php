...
class Api_signin extends MX_Controller
{
    protected $mono;
    protected $DEBUG;
    protected $logger;
    ...
    public function __construct()
    {
        ...
        $this->DEBUG       = $this->mono['vina_api_website']['signin']['debug'];
        $this->logger_path = $this->mono['vina_api_website']['signin']['logger_path'];
        $this->logger_file = $this->mono['vina_api_website']['signin']['logger_file'];
        $this->logger_name = $this->mono['vina_api_website']['signin']['logger_name'];
    }
    /**
     * API Xử lý đăng nhập
     *
     * @link /api/v1/utils/user/signin.html
     */
    public function index()
    {
        ...
        $password        = $this->input->get_post('password', true); // Mật khẩu đăng nhập tương ứng với số điện thoại
        ...
        $value_signature = md5($msisdn . $prefix . $password . $prefix . $token);
        if ($this->DEBUG === true)
        {
            $logger->info($getMethod . ' ' . current_url(), $input_params);
        }
        // filters
        if ($msisdn === null || $password === null || $signature === null)
        {
            $response = array(
                'Result' => 2,
        ...
        elseif ($value_signature !== $signature)
        {
            $response = array(
                ...
                'vail' => (ENVIRONMENT === 'production') ? null : $value_signature
            );
        }
        else
        {
            ...
            // Check dang nhap
            $user_info = $this->sub_model->check_info_subsmodel($data_check, true);
            if ($user_info > 0)
            {
                if ($services->onePack == 1)
                {
                    ...
                    unset($user_info);
                    // Lấy thông tin user
                    $user_info = $this->sub_model->check_info_subsmodel($data_check, false);
                    $response  = array(
                        'Result' => 0,
                ...
                {
                    // Nếu có 1 người dùng chỉ đăng ký được nhiều gói
                    $user_info = $this->sub_model->check_info_subsmodel($data_check, false, 0, false);
                    $response  = array(
                        'Result' => 0,
                        'Desc' => 'Đăng nhập thành công.'
                    );
    ...
    /**
     * Api_signin destructor.
     */
    public function __destruct()
    {
        $this->sub_model->close();
        log_message('error', 'API Signin for Website - Dong ket noi CSDL!');
    }
}
/* End of file Api_signin.php */
/* Location: ./based_core_apps_thudo/modules/Vinaphone-API-Website-Signin/controllers/Api_signin.php */
